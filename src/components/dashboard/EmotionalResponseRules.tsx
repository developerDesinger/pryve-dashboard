...
import { emotionalRulesAPI, type EmotionalRule } from "@/lib/api/emotionalRules";
...
export default function EmotionalResponseRules() {
  ...
  const [rules, setRules] = useState<EmotionalRule[]>([]);
  ...
  const loadEmotionalRules = useCallback(async () => {
    if (!user) return;
    ...
    try {
      ...
      const response = await emotionalRulesAPI.getEmotionalRules(
        token,
        currentPage,
        itemsPerPage,
      ...
      if (response.success && response.data) {
        ...
        showError(response.message || 'Failed to load emotional rules');
      }
    } catch (error) {
      console.error('Error loading emotional rules:', error);
      showError('Network error. Please try again.');
    } finally {
      setIsLoading(false);
    }
  }, [user, currentPage, itemsPerPage, selectedTrigger]);

  // Load emotional rules on component mount and when dependencies change
  useEffect(() => {
    loadEmotionalRules();
  }, [loadEmotionalRules]);
  ...
  const handleAddRule = async (newRule: {
    ...
    if (!newRule.trigger.trim()) {
      showError('Please enter a trigger for the emotional rule');
      return;
    }
    if (!newRule.responseType.trim()) {
      showError('Please enter a response type for the emotional rule');
      return;
    }
    if (!newRule.tone.trim()) {
      showError('Please enter a tone for the emotional rule');
      return;
    }
    if (!newRule.description.trim()) {
      showError('Please enter a description for the emotional rule');
      return;
    }
    
    setIsSaving(true);
    const loadingToast = showLoading('Creating emotional rule...');
    ...
    try {
      ...
      const response = await emotionalRulesAPI.createEmotionalRule(token, {
        trigger: newRule.trigger.trim(),
        responseType: newRule.responseType.trim(),
        tone: newRule.tone.trim(),
      ...
      if (response.success && response.data) {
        ...
        showSuccess('Emotional rule created successfully!');
        setRules(prev => [response.data!, ...prev]);
        setIsModalOpen(false);
      } else {
        dismiss(loadingToast);
        showError(response.message || 'Failed to create emotional rule');
      }
    } catch (error) {
      dismiss(loadingToast);
      showError('Network error. Please try again.');
  ...
  const confirmDeleteRule = async () => {
    ...
    setIsSaving(true);
    const loadingToast = showLoading('Deleting emotional rule...');
    ...
    try {
      ...
      const response = await emotionalRulesAPI.deleteEmotionalRule(token, ruleToDelete);
      
      if (response.success) {
        dismiss(loadingToast);
        showSuccess('Emotional rule deleted successfully!');
        setRules(prev => prev.filter(rule => rule.id !== ruleToDelete));
        setSelectedRules(prev => prev.filter(id => id !== ruleToDelete));
      } else {
        dismiss(loadingToast);
        showError(response.message || 'Failed to delete emotional rule');
      }
    } catch (error) {
      dismiss(loadingToast);
      showError('Network error. Please try again.');
  ...
  const confirmDeleteSelected = async () => {
    ...
    try {
      ...
      const response = await emotionalRulesAPI.deleteEmotionalRules(token, selectedRules);
      
      if (response.success) {
        dismiss(loadingToast);
        showSuccess(`${selectedRules.length} emotional rules deleted successfully!`);
        setRules(prev => prev.filter(rule => !selectedRules.includes(rule.id!)));
        setSelectedRules([]);
      } else {
        dismiss(loadingToast);
        showError(response.message || 'Failed to delete selected rules');
      }
    } catch (error) {
      dismiss(loadingToast);
      showError('Network error. Please try again.');
  ...
  return (
    <Card className="p-6 bg-white rounded-2xl border-0 space-y-6">
        ...
        {isLoading ? (
          <div className="flex items-center justify-center py-8">
            <div className="text-center">
              ...
              <p className="text-[14px] text-gray-500 mt-2">Loading emotional rules...</p>
            </div>
          </div>
        ) : (
              ...
              {rules.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-8 text-center text-gray-500">
                    No emotional rules found. Create your first rule to get started.
                  </td>
                </tr>
              ) : (
                rules.map((rule) => (
      ...
      <ConfirmationModal
        ...
        title={deleteType === 'single' ? 'Delete Emotional Rule' : 'Delete Selected Rules'}
        message={
          deleteType === 'single' 
            ? 'Are you sure you want to delete this emotional rule? This action cannot be undone.'
            : `Are you sure you want to delete ${selectedRules.length} selected emotional rule(s)? This action cannot be undone.`
        }
        confirmText="Delete"
        cancelText="Cancel"
        type="danger"
